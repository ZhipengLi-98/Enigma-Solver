...
class Enigma:
    ...
    def convert(self, message):
        ...
        for i in range(len(message)):
            ...
            passSec = self.second.forward(passFir, last=(self.first.init + i + 1) % 26, step=stepTwo)
            passThi = self.third.forward(passSec, last=self.second.init + stepTwo, step=stepThi)
            ...
            revFir = self.first.rev_forward(revSec, last=self.second.init + stepTwo, step=i + 1)
    ...
    def convert_char(self, c, step):
        ...
        passFir = self.first.forward(c, step=step + 1)
        stepTwo = self.first.check(step + 1)
        stepThi = self.second.check(stepTwo)
        passSec = self.second.forward(passFir, last=(self.first.init + step + 1) % 26, step=stepTwo)
        passThi = self.third.forward(passSec, last=self.second.init + stepTwo, step=stepThi)
        passRev = self.reverse[alphabet[(ord(passThi) - ord("A") - self.third.init - stepThi) % 26]]
        revThi = self.third.rev_forward(passRev, step=stepThi)
        revSec = self.second.rev_forward(revThi, last=self.third.init + stepThi, step=stepTwo)
        revFir = self.first.rev_forward(revSec, last=self.second.init + stepTwo, step=step + 1)
        final = alphabet[(ord(revFir) - ord("A") - self.first.init - step - 1) % 26]
